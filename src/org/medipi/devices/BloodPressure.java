...
public abstract class BloodPressure extends Device {
	...
	public String init() throws Exception {
		...
		if(medipi.isBasicDataView()) {
			...
			systolCol.setCellValueFactory(new PropertyValueFactory<>("systol"));

			TableColumn diastolCol = new TableColumn("Diastolic Pressure");
			diastolCol.setMinWidth(50);
			diastolCol.setCellValueFactory(new PropertyValueFactory<>("diastol"));

			TableColumn pulseRateCol = new TableColumn("Pulse Rate");
			pulseRateCol.setMinWidth(50);
			pulseRateCol.setCellValueFactory(new PropertyValueFactory<>("pulseRate"));

			TableColumn restCol = new TableColumn("Rest");
			restCol.setMinWidth(50);
			restCol.setCellValueFactory(new PropertyValueFactory<>("rest"));

			TableColumn arrhythmiaCol = new TableColumn("Arrhythmia");
			arrhythmiaCol.setMinWidth(50);
			arrhythmiaCol.setCellValueFactory(new PropertyValueFactory<>("arrhythmia"));
			...
			deviceWindow.getChildren().addAll(table);
		...
		systolHbox.getChildren().addAll(lastSystol, mmhg);
		// Diastolic reading
		...
		diastolHbox.getChildren().addAll(lastDiastol, mmhg2);
		// Heart Rate reading
		...
		pulseHbox.getChildren().addAll(lastPulse, bpm);
		VBox meterVBox = new VBox();
		meterVBox.setAlignment(Pos.CENTER);
		meterVBox.setId("resultsbox");
		meterVBox.setPrefWidth(200);
		meterVBox.getChildren().addAll(systolHbox, diastolHbox, pulseHbox);

		//create the main window HBox
		HBox dataHBox = new HBox();
		dataHBox.getChildren().addAll(dataBox, meterVBox);

		meterWindow.getChildren().addAll(dataHBox, new Separator(Orientation.HORIZONTAL));
	 ...
	 */
	protected void addDataPoint(Date d, int systol, int diastol, int pulseRate, boolean rest, boolean arrhythmia) {
		if(d.after(lastDate)) {
		...
		if(!medipi.isBasicDataView()) {
			MeterReading mr = new MeterReading(Utilities.DISPLAY_TABLE_FORMAT.format(d), String.valueOf(systol), String.valueOf(diastol), String.valueOf(pulseRate), String.valueOf(rest), String.valueOf(arrhythmia));
			data.add(mr);
		}
